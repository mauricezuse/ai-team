...
# Config flag for real Jira integration (set to False for now)
USE_REAL_JIRA = False
...
def stub_create_github_issue(title, body):
    logging.info(f"[GitHub] Would create issue: {title}\nBody: {body}")
    print(f"[GitHub] Would create issue: {title}")
    return {"url": f"https://github.com/your-org/your-repo/issues/123", "title": title}

def stub_create_jira_ticket(summary, description):
    url = f"https://your-domain.atlassian.net/browse/PROJ-123"
    if USE_REAL_JIRA:
        # Here you would call the real Jira API
        logging.info(f"[Jira] (REAL) Would create ticket: {summary}\nDescription: {description}")
        print(f"[Jira] (REAL) Would create ticket: {summary}")
        # Return the real ticket URL
        return {"url": url, "summary": summary}
    else:
        logging.info(f"[Jira] (STUB) Would create ticket: {summary}\nDescription: {description}")
        print(f"[Jira] (STUB) Would create ticket: {summary}")
        return {"url": url, "summary": summary}
...
def run_mvp_workflow(feature: str):
    ...
    try:
        ...
        # Only stub Jira integration after PM output
        jira_ticket = stub_create_jira_ticket(f"Feature: {feature}", pm_output)
        result["jira_ticket_url"] = jira_ticket["url"]
    ...
    while True:
        ...
        try:
            ...
            tester_output = tester_tool._openai_service.generate(tester_prompt, max_tokens=512)
